...
1. 保证可见性
   ...
   > **每次读取都要从主内存刷新最新的值到工作内存，用于保证看见其他线程所做的修改**
   >
   > **每次写完后立刻同步到主内存，让其他线程立刻看到自己对变量的修改**
   >
   > **避免指令重拍，通过加入内存屏障（指令重拍不能把后面的指令重排到内存屏障之前的位置）**
...
新增机制，修改版本号   **AtomicStampedReference**
...
![1567839903382](img_JUC多线程及高并发/1567839903382.png)

#### 重量级锁的实现

monitor，每个java对象自打娘胎里就有一个monitor对象，这个对象可能一开始就有，存储在每个对象的对象头中，也可能会在线程获取锁的时候自动生成

这个对象是用cpp实现的，结构如下：

![1567840066234](img_JUC多线程及高并发/1567840066234.png)

其中 owner 是指向持有这个ObjectMonitor的线程，WaitSet：等待池，EntryList ：锁池

当对象尝试进入锁 区域的时候，会首先进入EntryList 中 ，如果成功获取锁的了之后，owner会指向当前线程，并且count变量++，如果释放锁的时候，同理

如果线程调用wait方法，将释放锁，owner为null，并且进入等待池中，count--

在java虚拟机早期版本中，synchronized是重量级的锁，因为锁的操作需要依赖操作系统，这就需要从用户态转化成核心态，需要花费代价很大，但是从java1.6之后，虚拟机的开发人员对锁进行了大量的优化，其中有：
...
* 锁消除   
  ...
  > 消除没有必要的锁，比如在一个方法中的私有变量进行锁定操作（调用私有变量stringBuffer，此变量不会被多个线程持有，一定是线程安全的，所以jvm会进行消除锁）
...
> 如果一个线程获取了锁，锁就进入偏向模式，markword 锁标记为偏向（01），而且偏向锁中记录的线程Id为当前线程ID，这样就不用进行同步操作，直接进入代码块中执行

对象第一次获取线程，jvm会使用cas操作把线程ID记录在对象的mark word 上，如果操作成功，就哈哈哈

如果有另一个线程尝试获取锁的时候，偏向锁模式宣告结束，转化成轻量级锁



#### 轻量级锁

在代码进入同步代码块时，如果此时同步对象没有被锁定（01）状态，虚拟机将在当前线程的栈帧中简历一个名为锁记录的空间，用于存储对象的MW拷贝，然后通过cas的方式将对象的MW指向锁记录，如果成功了，则代表该对象拥有 了该锁，所标志位变成00，如果失败了，先检查是否该对象的MW指向当前线程的栈帧，如果是，直接执行，如果不是，说明该队形被其他线程抢占了，轻量级锁技术无法施展，膨胀为重量级锁。